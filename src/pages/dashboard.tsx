...
import { getShopManagers } from 'apiSdk/shop-managers';
import { ShopManagerInterface } from 'interfaces/shop-manager';

import { BusinessOwnerListPage as ShowCaseList } from 'pages/business-owners';
...
function DashboardPage() {
  ...
  const shopManagerStat = useSWR<PaginatedInterface<ShopManagerInterface>>(
    () => `/shop-managers`,
    () => getShopManagers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Shop Managers',
      result: shopManagerStat?.data?.totalCount,
      isLoading: shopManagerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`business-owners`} passHref legacyBehavior>
                  <Button
